...
function createDivs() {
  ...
  allHexCodes.forEach(function (code) {
    guesses.innerHTML += `<div id="${code}" style="background:${code}" class="guess">&nbsp;</div>`;
  });
  addListeners();
}

function addListeners() {
    document.querySelectorAll('.guess').forEach(item => {
        item.addEventListener('click', e => {
            if (e.target.getAttribute("id") === correctHexCode) {
                correctAnswer();
              }else{
                incorrectAnswer();
              }
...
function generateNewHexCode() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (letterOrNumber === 0) hexcode += "" + Math.floor(Math.random() * 10);
    else {
